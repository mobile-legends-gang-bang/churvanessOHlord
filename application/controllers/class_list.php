...
class Class_list extends CI_Controller {
	...
	public function index() {
		if(!$this->session->userdata('logged_in')) {
			...
			$data['title'] = "Edukit - Setup Class";
			$data['name'] = "SETUP NEW CLASS";
			$data['notesview'] = $this->class_list_model->getclasslist();
			$data['content'] = "class_list/index";
	...
	public function getclasslist(){
		if($this->session->userdata('logged_in')){
			$data = $this->class_list_model->getclasslist();
		...
		if($this->session->userdata('logged_in')) {
			...
			if ($this->form_validation->run()){
				...
				if($query->num_rows() > 0){
					...
					$data = $this->class_list_model->updatebehavior();
					$response['status'] = TRUE;
	...
	public function deletebehavior(){
		if($this->session->userdata('logged_in')){
			$data = $this->class_list_model->deletebehavior();
			$response['status'] = TRUE;
			$response['message'] = "Successfully saved behavior.";
			echo json_encode($data);
		} else
